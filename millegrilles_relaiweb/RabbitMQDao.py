...
class MqThread:
    ...
    def __init__(self, event_stop: Event, etat_relaiweb, command_handler: CommandHandler, routing_keys_consumers: list):
        ...
        self.__etat_senseurspassifs = etat_relaiweb
    ...
    async def configurer(self):
        ...
        res_relai_get = RessourcesConsommation(self.callback_reply_q, nom_queue='relaiweb/get', channel_separe=True, est_asyncio=True)
        res_relai_get.ajouter_rk(Constantes.SECURITE_PUBLIC, 'commande.relaiweb.get')
        messages_thread.ajouter_consumer(res_relai_get)
        res_relai_post = RessourcesConsommation(self.callback_reply_q, nom_queue='relaiweb/post', channel_separe=True, est_asyncio=True)
        res_relai_post.ajouter_rk(Constantes.SECURITE_PUBLIC, 'commande.relaiweb.post')
...
class RabbitMQDao:
    ...
    def __init__(self, event_stop: Event, etat_relaiweb):
        self.__logger = logging.getLogger(__name__ + '.' + self.__class__.__name__)
        self.__event_stop = event_stop
        self.__etat_relaiweb = etat_relaiweb

        self.__command_handler = CommandHandler(etat_relaiweb)
    ...
    async def creer_thread(self):
        ...
        routing_keys = [
            'commande.relaiweb.get',
            'commande.relaiweb.post',
        ]
