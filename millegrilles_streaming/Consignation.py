...
class ConsignationHandler:
    ...
    def __init__(self, stop_event: asyncio.Event, etat_instance: EtatStreaming):
        ...
        self.__queue_download: Optional[asyncio.Queue] = None

        self.__session_http: aiohttp.ClientSession = None

    async def configurer(self):
        self.__trigger_consignation = asyncio.Event()
        self.__queue_download = asyncio.Queue(maxsize=10)
    ...
    #     dir_download = os.path.join(configuration.dir_staging, 'download')
    #
    #     # Parcourir liste de domaines a synchroniser
    #     try:
    #         for path_item in os.listdir(dir_download):
    #             path_complet = os.path.join(dir_download, path_item)
    #             if os.path.isdir(path_complet) is False:
    #                 continue  # On veut juste les domaines
    #
    #             # Comparer liste locale avec serveur
    #             await self.queue_fichiers_a_downloader(uuid_backup, dir_download, path_item)
    #
    ...
    # async def queue_fichiers_a_downloader(self, uuid_backup: str, path_backup: str, domaine: str):
    ...
    async def entretien_consignation(self):
        ...
        configuration = self.__etat_instance.configuration
        dir_staging = configuration.dir_staging
